...
def getStateActions(states, point):
    actions = [ ]
    possibleActions = [ 'left', 'right', 'up', 'down' ]

    for action in possibleActions:
        target = getMovementResult(point, action)
        if target[0] >= 0 and target[0] < len(states) and target[1] >= 0 and target[1] < len(states[target[0]]) and states[target[0]][target[1]] != 1:
            actions.append(action)

    return actions

def getMovementResult(point, action):
    if action == 'left':
...
def getActionResults(states, sourcePoint, action):
    targetPoint = getMovementResult(sourcePoint, action)
    reward = 0

    if targetPoint[0] >= 0 and targetPoint[0] < len(states) and targetPoint[1] >= 0 and targetPoint[1] < len(states[0]) and states[targetPoint[0]][targetPoint[1]] == 2:
        reward = 1000000000
        ...
        for i in range(len(states)):
            for j in range(len(states[i])):
                ...
                for action in actions:
                    actionResults = getActionResults(states, point, action)
                    result = 0

                    for actionResult in actionResults:
                        result += actionResult[2] * (actionResult[1] + values[actionResult[0][0] + 1][actionResult[0][1] + 1])
    ...
    for i in range(len(states)):
        for j in range(len(states[i])):
            if states[i][j] == 1:
                ...
                values[i + 1][j + 1] = 1000000000

    for i in range(len(values)):
        values[i][0] = -1
        values[i][len(values[i]) - 1] = -1

    for j in range(len(values[0])):
        values[0][j] = -1
        values[len(values) - 1][j] = -1

    snakePoint = (snakePoint[0] + 1, snakePoint[1] + 1)
    bestPoint = (snakePoint[0] + 1, snakePoint[1])
    bestDirection = DIRECTIONS.Down

    if values[snakePoint[0] - 1][snakePoint[1]] > values[bestPoint[0]][bestPoint[1]]:
        bestPoint = (snakePoint[0] - 1, snakePoint[1])
        bestDirection = DIRECTIONS.Up

    if values[snakePoint[0]][snakePoint[1] - 1] > values[bestPoint[0]][bestPoint[1]]:
        bestPoint = (snakePoint[0], snakePoint[1] - 1)
